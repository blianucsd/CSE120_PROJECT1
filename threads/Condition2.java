...
public class Condition2 {
	...
	public Condition2(Lock conditionLock) {
		...
		waitQueue1 = new LinkedList<KThread>();
	}
	...
	public void wake() {
		...
		/*if(!waitQueue.isEmpty()) {
			KThread thread = waitQueue1.remove();
	 ...
	 */
		...
		Lib.assertTrue(conditionLock.isHeldByCurrentThread());
		System.out.println("sleepFor: " + KThread.currentThread());
    ...
    private static class B implements Runnable {
	    private KThread k;
	    B (KThread k) {this.k = k;}
	    public void run() {
		System.out.println("in run");
		ThreadedKernel.alarm.waitUntil(2940);
		ThreadedKernel.alarm.cancel(k);
	    }
    }
	...
	KThread next = new KThread (new B(k)).setName("next");
	//boolean intStatus = Machine.interrupt().disable();
	//next.ready();
	//Machine.interrupt().restore(intStatus);
